...
def register():
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            return redirect(url_for('auth.login'))  # 只在成功注册后重定向到登录页

        except Exception as e:
            db.session.rollback()
            flash("An error occurred. Please try again.", "danger")
            return render_template("register.html", form=form)  # 错误时渲染模板而不是重定向
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user.password == form.password.data:
            ...
            return redirect(url_for("homepage.ListAllSpaces"))  # 成功登录后跳转到主页
        else:
